...
export async function handle_tweet(options, result) {
  if (!RE.Tweet.test(options.url)) {
    return false;
  }

  result.video_url_list = await fetch_tweet_video_url_list(options);

  return true;
}

async function fetch_tweet_video_url_list(options) {
  const url_match = options.url.href.match(RE.Tweet);
...
function tweet_video_list(tweet) {
  ...
  for (const media of media_list) {
    ...
    for (const variant of variant_list) {
      if (variant.bitrate && (!max_variant || variant.bitrate > max_variant.bitrate)) {
        max_variant = variant;
      }
    }

    if (max_variant) {
      video_list.push(max_variant.url);
    }
  }
